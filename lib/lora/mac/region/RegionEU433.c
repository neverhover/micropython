...
bool RegionEU433RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    {
        modem = MODEM_LORA;
        //Radio.SetRxConfig( modem, rxConfig->Bandwidth, phyDr, 1, 0, 8, rxConfig->WindowTimeout, false, 0, false, 0, 0, true, false );
        Radio.SetRxConfig( modem, rxConfig->Bandwidth, phyDr, 1, 0, 8, rxConfig->WindowTimeout, false, 0, false, 0, 0, false, rxConfig->RxContinuous );
    }
    printf("[RegionEU433.c] RegionEU433RxConfig --> Freq:%d "
              "Radio.SetRxConfig( modem=%d, 0, bandwidth=%d, phyDr=%d,"
              "rxWindowTimeout=%d  \r\n",
              frequency, modem, rxConfig->Bandwidth,
              phyDr, rxConfig->WindowTimeout);
...
bool RegionEU433TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    printf("[RegionEU433.c] RegionEU433TxConfig --> Radio Channel:%d\r\n", Channels[txConfig->Channel].Frequency);
...
//        Radio.SetTxConfig( modem, phyTxPower, 0, 1, phyDr, 1, 8, false, true, 0, 0, false, 3000 );
    }

    printf("[RegionEU433.c] RegionEU433TxConfig --> Freq:%d "
        "Radio.SetTxConfig( modem=%d, phyTxPower=%d, 0, bandwidth=%d, phyDr=%d,"
        " 1, 8, false, true, 0, 0, false, 3000 )\r\n",
        Channels[txConfig->Channel].Frequency, modem, phyTxPower, bandwidth,
        phyDr);
...
bool RegionEU433NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    printf("[Region.c] RegionEU433NextChannel nextChanParams->AggrTimeOff:%d, nextChanParams->LastAggrTx:%d "
        "TimerGetElapsedTime( nextChanParams->LastAggrTx ):%d\r\n",
        nextChanParams->AggrTimeOff, nextChanParams->LastAggrTx,
    ...
    printf("[Region.c] RegionEU433NextChannel nbEnabledChannels:%d delayTx:%d\r\n",
        nbEnabledChannels, delayTx);
